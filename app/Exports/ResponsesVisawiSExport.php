...
class ResponsesVisawiSExport implements WithMultipleSheets
{
    use Exportable;
    ...
    public function sheets(): array
    {
        ...
        foreach ($this->survey_ids as $survey_id) {
            ...
            if ($survey) {
                $sheets[] = new VisawiSResponseSheet($survey_id, $survey->title);
            }
        }

        // If multiple surveys, add a summary sheet
        if (count($this->survey_ids) > 1) {
            $sheets[] = new VisawiSSummarySheet($this->survey_ids);
        }
...
class VisawiSResponseSheet implements FromCollection, ShouldAutoSize, WithHeadings, WithStyles, WithEvents, WithTitle
{
    use Exportable;
    ...
    {
        $scores = VisawiSScore::where('survey_id', $this->survey_id)
            ->with('surveyResponse')
            ->get();
...
class VisawiSSummarySheet implements FromCollection, ShouldAutoSize, WithHeadings, WithStyles, WithTitle
{
    use Exportable;
    ...
    public function collection()
    {
        ...
        foreach ($this->survey_ids as $survey_id) {
            $survey = Survey::find($survey_id);
            $scores = VisawiSScore::where('survey_id', $survey_id)->get();

            if ($scores->count() > 0) {
                $avgSimplicity = $scores->avg('simplicity');
                $avgDiversity = $scores->avg('diversity');
                $avgColorfulness = $scores->avg('colorfulness');
                $avgCraftsmanship = $scores->avg('craftsmanship');
                $avgFinalScore = $scores->avg('final_score');

                $summaries[] = [
                    'Survey Title' => $survey->title,
                    'Total Respondents' => $scores->count(),
                    'Avg Simplicity' => round($avgSimplicity, 2),
                    'Avg Diversity' => round($avgDiversity, 2),
                    'Avg Colorfulness' => round($avgColorfulness, 2),
                    'Avg Craftsmanship' => round($avgCraftsmanship, 2),
